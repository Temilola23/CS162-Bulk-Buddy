...
class Trip(db.Model):
    ...
    __table_args__ = (
        ...
        db.Index("ix_trips_status_coords", "status", "pickup_lat", "pickup_lng"),
    )

    # Relationships
    driver = db.relationship("User", back_populates="trips")
    items = db.relationship(
        "Item", back_populates="trip", lazy="dynamic", cascade="all, delete-orphan"
    )
    orders = db.relationship(
        "Order", back_populates="trip", lazy="dynamic", cascade="all, delete-orphan"
    )
